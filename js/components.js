...
(function () {
  ...
  /* ── 1. BODY TOP PADDING — clears the fixed navbar ── */
  body.style.paddingTop = '0';   /* navbar JS will handle offset via hero min-height */
...
<nav class="${navClass}" id="mainNav" aria-label="Main navigation">
  <div class="container d-flex align-items-center justify-content-between gap-3">
    ...
    <div class="collapse navbar-collapse" id="navMenu">
      <div class="d-flex flex-column flex-lg-row align-items-start align-items-lg-center
                  gap-2 gap-lg-4 pt-3 pt-lg-0 pb-3 pb-lg-0 w-100 justify-content-lg-end">
  ...
  /* ── 3. FOOTER ── */
  /* Guard: don't double-inject footer either */
...
<footer class="footer" id="siteFooter">
  ...
  function buildNavLinks(root, currentPage, currentTheme) {
    ...
    const mainLinks = links.map(({ href, label, key }) => {
      ...
      return `<a class="${linkClass}${isActive ? ' nav-active' : ''}" href="${href}">${label}</a>`;
    }).join('\n        ');
